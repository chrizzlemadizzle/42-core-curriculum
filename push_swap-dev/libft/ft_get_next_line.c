...
/*   Updated: 2024/09/23 14:31:54 by cdahne           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_parse_buffer(char *buffer, t_list *t_remainder, size_t chr_read)
{
	...
	if (len < 0 && chr_read == BUFFER_SIZE)
	{
		...
		ft_lstadd_back(&t_remainder, ft_lstnew(nodecontent));
	}
...
char	*ft_check_remainder(t_list *t_remainder)
{
	...
	if (!linebreak)
		return (NULL);
	len = linebreak - (char *)t_remainder->content + 1;
...
char	*ft_get_next_line(int fd)
{
	...
	while (!line)
	{
		...
		if (chr_read < 0 || chr_read > BUFFER_SIZE || (chr_read == 0 && \
			!*(char *)t_remainder.content && !t_remainder.next))
		{
			free(buffer);
			ft_clear_remainder(&t_remainder);
			return (NULL);
